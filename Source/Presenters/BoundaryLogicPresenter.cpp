...
{
    if (&editor == &cutInEditor) isEditingIn = true;
    else if (&editor == &cutOutEditor) isEditingOut = true;
...
void BoundaryLogicPresenter::textEditorReturnKeyPressed(juce::TextEditor& editor)
{
    ...
    if (&editor == &cutInEditor) applyCutInFromEditor(newPosition, editor);
    else if (&editor == &cutOutEditor) applyCutOutFromEditor(newPosition, editor);
...
void BoundaryLogicPresenter::textEditorEscapeKeyPressed(juce::TextEditor& editor)
{
    ...
    if (&editor == &cutInEditor) syncEditorToPosition(editor, owner.getAudioPlayer().getCutIn());
    else if (&editor == &cutOutEditor) syncEditorToPosition(editor, owner.getAudioPlayer().getCutOut());
...
{
    if (event.eventComponent == &cutInEditor) isEditingIn = true;
    else if (event.eventComponent == &cutOutEditor) isEditingOut = true;
}
...
void BoundaryLogicPresenter::mouseUp(const juce::MouseEvent& event)
{
    ...
    if (charIndex <= 1) newRange = juce::Range<int>(0, 2);
    else if (charIndex >= 3 && charIndex <= 4) newRange = juce::Range<int>(3, 5);
    else if (charIndex >= 6 && charIndex <= 7) newRange = juce::Range<int>(6, 8);
    else if (charIndex >= 9 && charIndex <= 11) newRange = juce::Range<int>(9, 12);
    else return;

    juce::MessageManager::callAsync([editor, newRange] {
        if (editor != nullptr) editor->setHighlightedRegion(newRange);
    });
}
...
void BoundaryLogicPresenter::mouseWheelMove(const juce::MouseEvent& event, const juce::MouseWheelDetails& wheel)
{
    ...
    const double delta = ((wheel.deltaY > 0) ? 1.0 : -1.0) * step;

    if (editor == &cutInEditor)
    {
        const double currentIn = owner.getAudioPlayer().getCutIn();
        setCutInPosition(currentIn + delta);
    ...
    {
        const double currentOut = owner.getAudioPlayer().getCutOut();
        setCutOutPosition(currentOut + delta);
...
void BoundaryLogicPresenter::syncEditorToPosition(juce::TextEditor& editor, double positionSeconds)
{
    if (editor.hasKeyboardFocus(true) || (&editor == &cutInEditor && isEditingIn) || (&editor == &cutOutEditor && isEditingOut))
        return;
...
void BoundaryLogicPresenter::setCutInPosition(double positionSeconds)
{
    ...
    if (!silenceDetector.getIsAutoCutInActive() && newPos >= currentOut && silenceDetector.getIsAutoCutOutActive())
        owner.setAutoCutOutActive(false);
    ...
    if (silenceDetector.getIsAutoCutInActive() && newPos >= currentOut)
    {
        ...
        if (silenceDetector.getIsAutoCutOutActive()) silenceDetector.detectOutSilence();
    }
...
void BoundaryLogicPresenter::setCutOutPosition(double positionSeconds)
{
    ...
    if (!silenceDetector.getIsAutoCutOutActive() && newPos <= currentIn && silenceDetector.getIsAutoCutInActive())
        owner.setAutoCutInActive(false);
    ...
    if (silenceDetector.getIsAutoCutOutActive() && newPos <= currentIn)
    {
        ...
        if (silenceDetector.getIsAutoCutInActive()) silenceDetector.detectInSilence();
    }
