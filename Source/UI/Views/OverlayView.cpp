...
OverlayView::OverlayView(ControlPanel& ownerIn)
    : owner(ownerIn)
{
...
void OverlayView::paint(juce::Graphics& g)
{
    ...
    float inX = (float)waveformBounds.getX() +
                 CoordinateMapper::secondsToPixels(cutIn, (float)waveformBounds.getWidth(),
                                                   audioLength);
    float outX = (float)waveformBounds.getX() +
                  CoordinateMapper::secondsToPixels(cutOut, (float)waveformBounds.getWidth(),
                                                    audioLength);
    ...
    // So getLocalArea(owner, ...) is the correct way to map from owner-relative to overlay-relative.
    auto mapFromOwner = [&](juce::Rectangle<int> r) {
        return getLocalArea(&owner, r);
    };
    ...
    auto drawConnector = [&](float x, juce::Component* btn, bool toTop) {
        ...
        g.drawLine(btnCenterX, btnBottomY, x, toTop ? (float)waveformBounds.getY() : (float)waveformBounds.getBottom(), 2.0f);
    };
    ...
    if (inStrip != nullptr) {
        std::vector<juce::Component*> inGroup = { 
            ...
            &owner.getSilenceDetector().getInSilenceThresholdEditor(), &inStrip->getAutoCutButton() 
        };
    ...
    if (outStrip != nullptr) {
        std::vector<juce::Component*> outGroup = { 
            ...
            &owner.getSilenceDetector().getOutSilenceThresholdEditor(), &outStrip->getAutoCutButton() 
        };
