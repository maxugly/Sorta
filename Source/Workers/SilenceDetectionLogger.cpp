...
void SilenceDetectionLogger::logReadingSamples(SilenceWorkerClient &client,
    ...
    client.logStatusMessage("SilenceDetector: Scanning " + juce::String(length) + " samples for " +
                            direction + " Silence Boundary.");
}

void SilenceDetectionLogger::logZeroLength(SilenceWorkerClient &client) {
    client.logStatusMessage("SilenceDetector: Audio length is 0, cannot detect Silence Boundaries.",
                            true);
}

void SilenceDetectionLogger::logCutInSet(SilenceWorkerClient &client, juce::int64 sampleIndex,
                                         double sampleRate) {
    client.logStatusMessage("Silence Boundary (Start) set to sample " + juce::String(sampleIndex) +
...
void SilenceDetectionLogger::logCutOutSet(SilenceWorkerClient &client, juce::int64 sampleIndex,
                                          double sampleRate) {
    client.logStatusMessage("Silence Boundary (End) set to sample " + juce::String(sampleIndex) +
                            " (" + TimeUtils::formatTime((double)sampleIndex / sampleRate) + ")");
}
...
void SilenceDetectionLogger::logAudioTooLarge(SilenceWorkerClient &client) {
    client.logStatusMessage(
        "SilenceDetector: Audio file is too large for automated Cut Point detection.", true);
}
